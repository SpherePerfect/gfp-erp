...
import { ServiceTemplate, PaymentMilestone, EngagementRecord } from '../types';
...
interface TemplateManagerProps {
  ...
  onOpenEngagement?: (record: EngagementRecord) => void;
...
export const TemplateManager: React.FC<TemplateManagerProps> = ({
  ...
  // Service template traceability history modal
  const [selectedTemplateForHistory, setSelectedTemplateForHistory] = useState<ServiceTemplate | null>(null);
                  ...
                  >
                    ...
                    <div className="w-8 h-8 flex items-center justify-center shrink-0">
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-none transition shrink-0"
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6">
      ...
      {templates.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {templates.map((template) => {
            const usedEngagements = (engagements || []).filter((eng) => {
              const svcs = eng.services && eng.services.length > 0 ? eng.services : [eng.service];
              return svcs.some(
                (s) =>
                  s &&
                  (s.id === template.id ||
                    s.serviceCode === template.serviceCode ||
                    (s.serviceTitle &&
                      template.serviceTitle &&
                      s.serviceTitle.toLowerCase() === template.serviceTitle.toLowerCase()))
              );
            });
            ...
            return (
              <div
                key={template.id}
                className="bg-white border border-slate-200 hover:border-blue-900 rounded-none shadow-2xs hover:shadow-sm transition-all duration-200 flex flex-col justify-between overflow-hidden group"
              >
                <div className="p-5">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <span className="px-2.5 py-1 bg-blue-50 text-blue-950 border border-blue-200 font-mono text-xs font-semibold rounded-none">
                  ...
                  <h3 className="font-semibold text-slate-900 text-sm leading-snug mb-1.5 group-hover:text-blue-950 transition-colors">
                    {template.serviceTitle}
                  </h3>
                  ...
                  {/* Template Usage in Engagement Letters Badge & Button */}
                  <button
                    type="button"
                    onClick={() => setSelectedTemplateForHistory(template)}
                    className="w-full mt-3 py-1.5 px-2.5 bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-300 text-xs font-semibold text-slate-700 hover:text-blue-950 flex items-center justify-between transition cursor-pointer"
                    title={`View all ${usedEngagements.length} Engagement Letters that used this service template`}
                  >
                    <span className="flex items-center gap-1.5">
                      <Briefcase className="w-3.5 h-3.5 text-blue-900" />
                      <span>Used in <strong className="text-blue-950">{usedEngagements.length}</strong> EL{usedEngagements.length === 1 ? '' : 's'}</span>
                    </span>
                    <span className="text-[11px] text-blue-900 font-bold hover:underline flex items-center gap-0.5">
                      <span>View List</span>
      ...
      {/* 3. Modal: Service Template Traceability - List of all ELs using this template */}
      {selectedTemplateForHistory && (() => {
        const matchingEngagements = (engagements || []).filter((eng) => {
          const svcs = eng.services && eng.services.length > 0 ? eng.services : [eng.service];
          return svcs.some(
            (s) =>
              s &&
              (s.id === selectedTemplateForHistory.id ||
                s.serviceCode === selectedTemplateForHistory.serviceCode ||
                (s.serviceTitle &&
                  selectedTemplateForHistory.serviceTitle &&
                  s.serviceTitle.toLowerCase() === selectedTemplateForHistory.serviceTitle.toLowerCase()))
          );
        });

        const totalFees = matchingEngagements.reduce((sum, eng) => {
          const svcs = eng.services && eng.services.length > 0 ? eng.services : [eng.service];
          ...
          return sum + (match?.pricing?.feeAmount || 0);
        }, 0);

        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-backdrop-in">
            <div className="bg-white border border-slate-200 shadow-2xl max-w-3xl w-full flex flex-col max-h-[90vh] animate-modal-in overflow-hidden">
              {/* Modal Header */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-blue-900 bg-[#0B2545] text-white">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-none bg-blue-900 border border-blue-800 flex items-center justify-center text-blue-100">
                    <Briefcase className="w-4 h-4" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-base font-bold text-white tracking-tight">
                        Template Usage Traceability
                      </h2>
                      <span className="px-2 py-0.5 bg-blue-950 border border-blue-800 text-blue-200 font-mono text-[11px] font-semibold">
                        {selectedTemplateForHistory.serviceCode}
                      </span>
                    </div>
                    <p className="text-xs text-blue-200 font-normal">
                      Full historical list of Engagement Letters generated using &ldquo;{selectedTemplateForHistory.serviceTitle}&rdquo;
                    </p>
                  </div>
                </div>
                <button
                  ...
                  className="w-7 h-7 flex items-center justify-center text-blue-200 hover:text-white hover:bg-white/10 transition-colors"
                  title="Close Window"
                >
                  <X className="w-4 h-4" />
                </button>
              ...
              {/* Stats Bar */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 px-6 py-3 bg-slate-50 border-b border-slate-200 text-xs">
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-slate-500 font-medium">Linked Advisory Fee:</span>
                  <span className="font-bold text-blue-950 font-mono text-sm">
                    {formatIndianCurrency(totalFees)}
                  </span>
                </div>
                <div className="flex items-center gap-2 justify-start sm:justify-end">
                  <button
                    type="button"
                    onClick={() => {
                      const tmpl = selectedTemplateForHistory;
                      setSelectedTemplateForHistory(null);
                      onSelectTemplateForNewEngagement(tmpl);
                    }}
                    className="text-[11px] font-bold text-blue-900 hover:text-blue-950 underline flex items-center gap-1"
                  >
                    <span>+ New Letter with this Template</span>
                  </button>
                </div>
              </div>
                ...
                {matchingEngagements.length === 0 ? (
                  <div className="text-center py-12 px-4 border border-dashed border-slate-200 bg-slate-50/50">
                    ...
                    <p className="text-xs text-slate-500 max-w-md mx-auto mb-4">
                      This service template has not been attached to any saved engagement letters yet. You can create the first letter now.
                    </p>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="px-4 py-2 bg-[#0B2545] hover:bg-[#133863] text-white text-xs font-semibold shadow-xs inline-flex items-center gap-1.5"
                    >
                      <Plus className="w-3.5 h-3.5" />
                      <span>Create Letter with this Template</span>
                    </button>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-200 bg-white">
                        {matchingEngagements.map((eng) => {
                          ...
                          const fee = matchedSvc?.pricing?.feeAmount || eng.service?.pricing?.feeAmount || 0;

                          return (
                            <tr key={eng.id} className="hover:bg-blue-50/40 transition-colors">
                              <td className="py-2.5 px-3 font-mono font-semibold text-slate-800 whitespace-nowrap">
                                <div>{eng.referenceNumber}</div>
                                <div className="text-[10px] text-slate-400 font-normal font-sans">{eng.date}</div>
                              </td>
                              <td className="py-2.5 px-3">
                                <div className="font-bold text-slate-900">{eng.client?.companyName || 'Unnamed Client'}</div>
                                <div className="text-[11px] text-slate-500">{eng.client?.addresseeName || ''}</div>
                              </td>
                              <td className="py-2.5 px-3 text-slate-600 whitespace-nowrap">
                                {eng.signatory?.name || 'Authorised Signatory'}
                              </td>
                              ...
                              <td className="py-2.5 px-3 text-center whitespace-nowrap">
                                {onOpenEngagement ? (
                                  <button
                                    ...
                                    className="px-2.5 py-1 bg-blue-50 hover:bg-[#0B2545] text-blue-900 hover:text-white border border-blue-200 hover:border-[#0B2545] font-semibold text-[11px] transition-colors inline-flex items-center gap-1"
                                  >
                                    <span>Open EL</span>
                                    <ExternalLink className="w-3 h-3" />
                                  </button>
              ...
              <div className="flex items-center justify-between px-6 py-3 border-t border-slate-200 bg-slate-50">
                <span className="text-xs text-slate-500">
                  Total {matchingEngagements.length} letter{matchingEngagements.length === 1 ? '' : 's'} linked to this template
                </span>
                <button
                  type="button"
                  onClick={() => setSelectedTemplateForHistory(null)}
                  className="px-4 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-300 text-xs font-semibold transition"
                >
                  Close
                </button>
              </div>
